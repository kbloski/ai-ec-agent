...
class MarketingStrategyRepository:
    ...
    def update(self, item: MarketingStrategy) -> MarketingStrategy:
        existing_item = self.db.query(MarketingStrategy).filter(MarketingStrategy.id == item.id).first()

        if not existing_item:
            raise ValueError(f"MarketingStrategy with id {item.id} not found")
    ...
    def delete(self, id: int) -> bool:
        item = self.db.query(MarketingStrategy).filter(MarketingStrategy.id == id).first()
